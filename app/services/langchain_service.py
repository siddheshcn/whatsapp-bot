...
def generate_langchain_response(prompt_text, template=None):
    ...
    intent_detection_chain = (ChatPromptTemplate.from_messages([
        ("system",
         "You are a helpful assistant that identifies the user's intent."),
        ("human",
    ...
    message_parsing_chain = (ChatPromptTemplate.from_messages([
        ("system",
         """You are a JSON formatting assistant. You MUST return a valid JSON object and NOTHING else.
    ...
    yt_summarization_chain = (ChatPromptTemplate.from_messages([
        ("system",
         "You are a helpful assistant that responds to user queries or requests regarding youtube videos."
         ),
        ("human",
         "The user asks {yt_conditions}. Youtube Video: {yt_transcript}")
    ])
                              | llm
                              | StrOutputParser())
    ...
    def process_user_message(user_message: str) -> str:
        ...
        try:
            ...
            intent_result = intent_detection_chain.invoke(
            ...
            if "youtubelink" in intent:
                ...
                try:
                    parsing_result = message_parsing_chain.invoke(
                        {"user_message": user_message})
                    ...
                    log_progress(
                        f"YouTube link: {yt_url}, User request: {yt_conditions}"
                    )
